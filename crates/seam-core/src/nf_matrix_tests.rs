...
mod tests {
    ...
    fn peer(n: u8) -> PeerId { PeerId([n; 16]) }
    fn res(n: u8) -> ResourceId { ResourceId([n; 16]) }
    fn tid(n: u8) -> TransferId { TransferId([n; 16]) }
    fn key(n: u8) -> AuthorityKey { AuthorityKey::Resource(res(n)) }

    fn setup() -> (FabricState, PeerId, PeerId) {
        let mut s = FabricState::new(Limits::default());
        let a = peer(1); let b = peer(2);
        s.add_peer(a).unwrap(); s.add_peer(b).unwrap();
        (s, a, b)
    }

    #[test] fn nf_01_reject_before_accept() {
        let (mut s, a, b) = setup();
        let k = key(1); s.register_authority(k, a).unwrap();
        s.offer_bundle(a, b, tid(1), vec![(k, res(1), 2, true)]).unwrap();
    ...
    #[test] fn nf_07_duplicate_native() {
        let (mut s, a, b) = setup();
        let k = key(1); s.register_authority(k, a).unwrap();
        s.offer_bundle(a, b, tid(1), vec![(k, res(1), 2, true)]).unwrap();
        ...
        assert!(s.mark_fabric_escrowed(a, tid(1), 0).is_err() || s.transfers.status(&tid(1)) == Some(BundleState::Accepted));
    }

    #[test] fn nf_08_late_after_commit() {
        let (mut s, a, b) = setup();
        let k = key(1); s.register_authority(k, a).unwrap();
        s.offer_bundle(a, b, tid(1), vec![(k, res(1), 2, true)]).unwrap();
        s.accept_bundle(b, tid(1)).unwrap();
